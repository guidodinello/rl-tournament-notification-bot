...
recreate() {
    $SSH "cd $REMOTE_DIR && docker rm -f rlbot; docker run -d --name rlbot --restart unless-stopped --network=host --env-file .env --log-opt max-size=10m --log-opt max-file=3 rlbot"
